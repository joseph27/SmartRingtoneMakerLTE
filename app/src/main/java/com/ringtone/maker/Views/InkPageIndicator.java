...
public class InkPageIndicator extends View implements ViewPager.OnPageChangeListener,
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        if (isAttachedToWindow) {
            ...
            if (currentPosition != position) {
                ...
                // new page but we're notification_ic ready to switch our 'current' page yet so adjust for that
                if (fraction == 1f) {
    ...
    public void onPageSelected(int position) {
        ...
        if (isAttachedToWindow) {
            ...
            // when notification_ic attached, don't animate the move, just store immediately
            setCurrentPageImmediate();
        }
    }
